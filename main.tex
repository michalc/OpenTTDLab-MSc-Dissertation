...
The aim of OpenTTDLab is to help studies that run OpenTTD to generate results without a human player; 6 existing studies have been found that do this: the work of Geert Beuneker et al \cite{beuneker2019autonomous}, Frank Bijlsma \cite{bijlsma2014evolving}, Geert Konijnendijk \cite{konijnendijk2015mcts}, Ond{\v{r}}ej {Lakom{\`y} \cite{lakomy2020railroad}, Luis Henrique Oliveira Rios and Luiz Chaimowicz \cite{rios2009trains}, and Maciej Wisniewski and Carsten Witt \cite{wisniewski2011artificial}. Informally reviewing these in terms of how they have seemingly achieved repeatability (the domain of the original researchers) or help achieve reproducibility and replicability (the domain of independent researchers), keeping in mind the barriers of the previous section and my own knowledge of the capabilities of OpenTTD, I found 8 problems. Against each is list which of the 3Rs that this effects is listed, but as discussed earlier this would then cascade: repeatability problems affect reproducibiliy and replicabiliy, and reproducibiliy problems affect replicabiliy.

\begin{enumerate}
\begin{descitem}{Low number of repetitions - repeatability}
Where there is no one-size fits all number to how many repetitions an experiments must have, some of the studies I would argue have repetitions that are so few they call into question the results that are based on them. The 2 experiments in \cite{rios2009trains} were run 7 times each, and the experiments of \cite{wisniewski2011artificial} were run just 3 times for example.
\end{descitem}
\begin{descitem}{Non-comparable repetitions - repeatability}
The experiments of \cite{rios2009trains} were run 14 times, but not for the same amount of in-game time. This calls into question whether these were even meaningful repetitions of the same experiment.
\end{descitem}
\begin{descitem}{Manual repetitions of experiments - repeatability}

\end{descitem}
\begin{descitem}{Insufficient artifacts supplied - Repeatability}
One of the most complex and impressive studies is \cite{bijlsma2014evolving}, using dynamic scripting to evolve the code of an OpenTTD AI using a genetic algorithm. In total it seems that there were at least 1250 experiments in total: 50 experiments each for 25 generations of the genetic algorithm, and after each of these in game metrics must have been extracted. There is nothing in OpenTTD that allows this to be done automatically, and it seems infeasible to have run this manually. The study does not explain how this was done, and so I suspect there were artifacts created not even mentioned in the study. Although far fewer iterations in its final experiments, I suspect \cite{konijnendijk2015mcts} must also have created artifacts when it compared its AI with existing AIs over 100 runs of an experiment.

\cite{konijnendijk2015mcts} more directly suggests artifacts were created - additions to OpenTTD itself. Although described, these are not available.
\end{descitem}
...
\end{descitem}
\begin{descitem}{No random seeds reported - repeatability}
...
\begin{descitem}{Insufficient detail of the high level algorithms - replicability}
\end{descitem}
...
It can be costly to repeat experiments, or to make it reproducible and replicable; and there is unfortunately insufficient incentive to address these issues as a matter of course. Indeed, in all of the found research that uses OpenTTD to run experiments without human involvement, I identified 7 issues on this front, with no research completely issue-free. A framework to reduce the cost of these should help similar future research avoid adding to the well-known replication crisis, as will be discussed in the following chapters.
...
The framework presented here allows experiments to be conducted with OpenTTD in a manner that is repeatable and reproducible. Reproducibility is up to the author's descriptions of the artifacts they created - they must be sufficient to recreate the artifacts, and so beyond the scope of the framework here. However, it should at least ease issues due to creating a similar enough experimental setup.
