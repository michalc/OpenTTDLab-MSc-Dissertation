...
\section{A brief history of simulation games in research}
...
Games have a long history of being used as tools beyond recreation, specifically as tools to simulate the real world in order to take conclusions about the game environment and apply them to the world outside of the game \cite{mayer2009gaming}. Far before the advent of computers, this goes back to even ancient times where war games were used to ZZZ [REF]. These eventually moved to logistic simulations, albeit still military based MONOPOLOGS \cite{jackson1959learning} that is often credited as the first computerised logistics simulation game - SAY WHEN AND WHAT USED FOR. Then the MIT beer game - to illustrate something in supply chains. These then evolved to games that informed policy making - EXAMPLE.
