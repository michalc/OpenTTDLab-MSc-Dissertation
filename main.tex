...
OpenTTD has been used to run experiments to help research algorithms in a variety of fields. However from my own review from reading the studies, not a single study has been found that does not appear to have problems in at least one of the 3Rs of repeatability, reproducibility or replicability. Given the wider replication crisis in many fields of science, this is a trend that should not continue, and a need to address this in future research using OpenTTD seems clear.

As shown in Chapter X OpenTTD provides a rich and flexible environment.

The development of

From the first set of experiments in Chapter X I have further shown the difficulty in - attempting to reproduce existing results and failing. However, using OpenTTDLab and providing the artifacts for running an analysing results, I have shown that it is straightforward to repeat experiments, for others to reproduce my work.
...
However, the biggest limitation in all of this work is that while I developed the framework in a highly agile mechanism in parallel with conducting the experiments of Chapters X-X, with a view to avoiding the issues of existing research discussed in Chapter 1, reproducibility and replicability is the domain of multiple sets of authors - those originally conducting the research and those then attempting to reproduce or replicate it. To fully validate that OpenTTDLab can be a useful tool in this, studies. I have provided 3 sets of experiments, but it remains to be seen if these are truly reproducible or replicable.
