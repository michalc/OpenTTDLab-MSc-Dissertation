...
OpenTTDLab also appears novel: I was unable to find any project takes an existing game created for recreation, and augments it into a reusable simulator that allows ranges of algorithms to run and results extracted without human involvement. The closest I found was Axelrod \cite{knight2016open}, a framework for investigating the Iterative Prisoner's Dilema, created in response to similar reproducibility issues identified in the respective literature to the issues described in Chapter \ref{chapter:introduction}. I discovered Axelrod towards the end of the current work, but there are a number of similarities to OpenTTDLab: it uses Python, it includes tests that run on every change, it can leverage multiple CPU cores, and it focuses not only reproducibility but also on reusability by allowing future users to use it with novel strategies, much like OpenTTDLab supports novel AIs. An interesting difference is that Axelrod includes functions to visualise results, but OpenTTDLab supports them only by returning the raw data to be visualised separately. I suspect a similar set of functions to generate visualisations would be a worthwhile addition to OpenTTDLab.

There are numerous other ways that OpenTTDLab can be improved. Improving scalability has already been suggested, but there are non-performance features as well. For example, OpenTTDLab does not extract relating to the field of play itself, which includes how roads and railways are connected. And while in many ways the results returned from OpenTTDLab are self-documenting, and there are examples presented in this work and in the OpenTTDLab documentation, in general the meaning of the values in results are not completely clear, and no standalone exhaustive documentation of what is currently extract from OpenTTD using OpenTTDLab currently exists. In addition, the results from OpenTTDLab are ephemeral: they exist only in memory and designing a format to persisting them to disk I deemed outside the scope of the project, because how to do so is not trivial. Axelrod has done this, but the data structures involved in the Iterative Prisoner's Dilemma are much simpler.


\chapter{Conclusion}
\label{chapter:conclusion}

While there are many improvements that can be made in OpenTTDLab and further investigations performed in terms of whether it does in fact achieve its aims, through this work I have provided evidence that OpenTTDLab provides the foundations of a reusable framework for conducting repeatable, reproducible, and replicable research. And I have provided evidence that even through simple experiments, OpenTTD can produce interesting results that appear to have real-world meanings. OpenTTDLab is already a useful tool, and I am extremely optimistic that it can become even more useful if it's improved  further.
