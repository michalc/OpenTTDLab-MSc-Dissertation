...
An alternative would be to output Pandas data frame(s) directly from OpenTTDLab. However, the main downside of this approach is that I would have to design a structure for the DataFrame(s), but as described OpenTTD savegame data is hierarchical, and there is no unique or one-size-fits-all approach to convert hierarchical data into two dimensional data frames: any approach would have pros and cons, and to investigate these I decided was out of scope of the current work. For similar reasons I rejected saving the results to non-ephemeral file such as SQLite: a tempting target since SQLite is is a recommended format by the US Library of Congress. While this would avoid the downside of the ephemeral nature in-memory Pythion lists and dictionaries in memory, it does not forbid the users from saving data, for example as simple CSV files as I chose to in Chapters..., it would require choices to be made on how to conversion of the hierarchical data to the relational of SQLite.
...
OpenTTDLab leverages multiple CPU cores: it starts a process per experiment to run OpenTTD and to extract data from its savegame files and then aggregate all the results back to the starting process. By default the number of processes as the number of CPU cores on the system, but this can be configured.

This was done due to my being frustrated by run times when iterating over the features of OpenTTDLab and running initial versions of the experiments of Chapters \ref{chapter:experiments-attempt-at-reproducing} \& \ref{chapter:experiments-simple-parameterised-ai}.

\section{Documentation}

... saves screenshots! Mostly just for me. I'm actually not sure how to link this to the main arguments... hmmm. Maybe mention the logo here?
...
The issue of difficuly of replicating results is not directly addressed by features of OpenTTDLab and relates to how existing research does not have information to replicate results. However, to mitigate this, how to address this is mentioned in the documentation of OpenTTDLab, as well as suggestions how how to use OpenTTDLab and publish results to ensure both repeatability and reproducibility of results.
