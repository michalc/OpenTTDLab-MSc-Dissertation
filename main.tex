...
This chapter explores this model and these abilities in more detail, as well other related existing features that allow OpenTTD to be controlled programatically and ultimately used, through the framework presented in the next chapter, to run simulations. This chapter also shows that the environment of OpenTTD is a rich one; while there are clearly some unrealistic aspects, it is at least a tempting target for running simulations that could extract some insights applicable to the real world.
...
On startup of a new game/simulation, there are classes of startup. A pre-defined scenario can be started, where the map is pre-defined...

In the random case, deterministically based on the seed and configuration, a pseudo-random world is generated. This is the land, height, trees, coastlines and sea. Also the sizes and positions of town, and the positions and types of industries are all pseudo-randomly generated.

This determinism helps OpenTTD be used for repeatable experiments.
...
I conducted a brief investigation into how well OpenTTDLab scales with the number of worker processes by running the same set of 50 simulations, but distributing the work between 1 to 8 worker processes, and seeing how the runtime changes. I also did these tests when passing the full set of results back to client code, and for only passing a minimal amount of data: the date, the company value at that date, and if an error occurred. All the simulations were conducted on a 2020 Apple M1 with 8 CPU cores, 16GB of RAM, OpenTTDLab 0.0.72, OpenTTD 13.4, Python 3.11.0, trAIns 2.1 with MD5 hash of c4c069dc797674e545411b59867ad0c2, and running for seeds 0 to 49, and each for a total of 1465 in-game days. The Python code to run the experiments can be seen Appendix \ref{chapter:scaling-running-code}, and the code to subsequently process and visualise the results in \ref{chapter:scaling-analyis-code}.
...
plot 'notebooks/01_scaling_results_02_wallclock_times.csv' \ 
   using 1:2
\end{gnuplot}
\caption{Wallclock time}
\label{figure:scaling-wallclock-time}
\end{figure}

As can be seen from Figure \ref{figure:scaling-wallclock-time}, runtime decreases as the number of processors increases, and so there is some useful parallelisation in terms of reducing the time for simulations. However, the amount that speedup increases as a processor is added decreases as the number of processors increases. And more specifically, up to 4 processors the scaling is just under linear, but after 4 processors the scaling is much worse. It can also be seen that the runtime is better when only a small amount of data is returned from the worker processes, but the scaling profiles are almost identical

A reason for the noted difference when the number of processors is greater than 4 comes from noting that the cores are not identical. According to Apple [REF] 4 cores are high performance, but the other 4 are lower performance (but have higher efficiency in terms of power usage). The scaling profiles found appears roughly consistent with the high/lower performance split, and suggests that the high performance cores being used in preference to the low performance cores.

Another reason comes from Amdhal's Law that describes how speedup is limited by the serial part of the program. This law is usually formulated as
...
This suggests that a way of increasing this limit is to reduce proportion of serial code. One part that is effectively not parallelisable is the copy of data from the worker process back to the main process - the main process's role in this will be serial. As can be seen from Figure X, reducing the amount of data returned does appear to increase the speedup.

To eliminate this copying entirely, the program could use threads instead of processes, because threads all use the same address space. However, Python suffers from the well-known Global Interpreter Lock (GIL) which means that even in multi-threaded code, only one thread can run at any one time, which could unfortunately increase the proportion of code that cannot be parallelised, and so from Amdhal's Law make scaling worse. Although the simulation itself is run using OpenTTD, there are parts of the algorithm done in Python: for example parsing the savegame files are done from Python code, and so subject to the GIL, effectively serial even if run from different threads, and so would reduce scaling potentially very drastically according to Amdhal's law.

More precise investigation, for example into the trade offs between the GIL and copying that multiple processes requires, and possible subsequent improvements to OpenTTDLab, is left to further work.
