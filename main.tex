...
However, even if there is sufficient detail, in all cases bar one they are not presented in a way that I would argue is ideal for replication: the detail is spread throughout the work, and not presented as one or algorithms that be replicated. The exception is the work of {Lakom{\`y}} \cite{lakomy2020railroad}: it includes many algorithms each of them explained and discussed, although they are presented as code rather than pseudo-code, and so if used could muddy the water between reproducing and replicating the results.
\end{descitem}
...
It can be costly to repeat experiments, or to make it easily reproducible and replicable; and there is unfortunately insufficient incentive to address these issues as a matter of course by researchers or by publishers. Indeed, in all of the found research that uses OpenTTD to run experiments without human involvement, I identified 7 issues on this front, with no research completely issue-free. A framework to reduce the cost of these, ideally in a reusable way, should help similar future research avoid adding to the well-known replication crisis. Such a framework is OpenTTDLab, as will be discussed in the following chapters.
