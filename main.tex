...
Simulation games have a long history of being used to inform government policies, see \cite{raghothama2013review} for a summary that focuses on logistics and simulation. In spite of this, no reference of using OpenTTD in this way has been found. A reason in \cite{raghothama2013review} is suggested: while some of the transportation aspects of the networks in OpenTTD are realistic, its economic model is not. In addition, if using OpenTTD as a simulator, any correspondence between OpenTTD and what it is simulating has not been determined: it has not been \emph{validated} \cite{doi:10.1177/1046878198291003}.

Of all these, using OpenTTD as a tool for researching algorithms is the primary focus of this project, and specifically the focus is the development of a tool, OpenTTDLab, that augments its existing features for repeatable, replicable, \& reproducible research. Given what is now the well-known replication crisis/replicability crisis/reproducibility crisis \cite{baker20161}, if there are problems with how such research is conducted using OpenTTD, as I will argue, then the usefulness of such a tool is clear.
...
The terms \emph{repeatability}, \emph{replicability}, and \emph{reproducibility}, the so-called \emph{3Rs}, unfortunately do not historically have universally agreed meanings \cite{plesser_reproducibility_2018} and to add to the confusion, the Association for Computing Machinery (ACM) in 2020 swapped their definitions reproducibility and replicability \cite{association_for_computing_machiner_new_2020}. For the avoidance of doubt, I use the versions defined in ACM's \emph{Artifact Review and Badging Version 1.1} \cite{association_for_computing_machiner_artifact_2020}. The definition is given in terms of artifacts:
...
\end{quote}
ACM's definitions of the 3Rs can be summarised as follows: for some results of an experiment conducted by a team of authors, where some computer-based artefacts were created to conduct these:

\begin{description}
\item[Repeatability] The authors can obtain the same results, using the same artefacts.
