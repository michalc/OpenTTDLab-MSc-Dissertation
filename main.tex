...
I considered a number of features while creating OpenTTD

Saving results to a file

\section{User facing interface}

The user facing interface consists primarily of a single function

\begin{description}
\item[Version of OpenTTDLab and OpenGFX] fffff
\item[Random seed] fffff
\item[Number of days] fffff
\item[The AIs to run] Specified by local folder of the code of the AI, bananas identifier, or GitHub repository (which can contain the commit ID)
\item[Parameters for each AI] fffff
\item[Maximum number of processes to run] fffff
\end{description}

The function returns a list, returning the results of each experiement. 

It also gives feedback. And saves screenshots.
...
The core feature of OpenTTDLab that allows is, in my opinion, the ease at which experiments can be run, with the random seed specified and results extracted at deterministic points of time in the game.

The parallelisation adds to the ability to make an experiment repeatable - simply by allowing more experiments to be run within any given amount of time.

\section{Replicability}

The seed, reference to specific versions of OpenTTD

\section{Reproducibility}

The seed, reference to specific versions of OpenTTD, but can refer to a folder for the AI.
...
The process I followed used my own feedback from creating the results of Chapter 4, which are essentially repeating experiments rather then replicating or reproducing the results of other teams'. As a result the design of the API is optimised for this.

It's not possible, or I think useful, to detail all the alternative APIs considered - some existed in for just a few seconds, either in documentation or just in my own head, before discarding them, and so there is no record of them. However, there are records of some initial designs of the earliest APIs I considered https://github.com/michalc/OpenTTDLab/pull/8/files that I think is indicative - it contained a multi stage process that focused on different.

I discarded this in favour of simpler single stage process to repeat experiments. I discovered it made simple repeating of a single experiment more frustrating that I think it should have been https://github.com/michalc/OpenTTDLab/pull/51. I realised also that also the repeating or replicating team themselves have to repeat experiments.
