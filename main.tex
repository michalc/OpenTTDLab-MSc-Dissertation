...
\end{gnuplot}
\caption{Total wallclock time of when running 50 experiments using OpenTTDLab over a varying number of processes, comparing returning all data to the controlling process, and returning a minimal amount of data. Using more worker processes results in a shorter runtime: for example using 8 worker processes uses about 20\% of the runtime as using 1 worker process. Returning a minimal amount of data also results in shorter runtime: approximately 30\% faster for each number of processes in the experiment.}
...
A reason for the difference in scaling when the number of processors is greater than 4 could come from that the cores are not identical. According to Apple [REF] 4 cores are high performance, but the other 4 are lower performance (but have higher efficiency in terms of power usage). The scaling profiles found appears roughly consistent with the high/lower performance split, and suggests that the high performance cores being used in preference to the low performance cores - although this is certainly not conclusive.

Another reason for the deviation from linear speedup from Amdhal's Law that describes how speedup is limited by the serial part of the program. This law is usually formulated as
