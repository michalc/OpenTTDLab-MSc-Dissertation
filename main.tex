...
The only aspect of the results that is easily explained is the higher company values for flat terrain rather than mountainous. As discussed in must be more investment in the rail infrastructure, and there is nothing in the economic model of OpenTTD to offset this by providing high incoming, in fact it is the opposite: the routes would be longer, trains go slower when going uphill or round corners, which increase the time to transfer goods and so reduces income compared to a a similar route but over flat terrain.

The distribution getting wider and wider over time... akin to a random walk?? Or could there be cumulative effects, some small decision at the start of the game effects more and more as time goes on.

The random aspect of the game is the initial world generation - could some worlds be more favourable than others, can this be predicted ahead of time even? From my own experience of playing OpenTTD for recreation, I do a form of "seed hacking" after the world is generated

The other aspects of the results are left unfortunately to speculation.

As noted in X how to deal with errors is often underreported, and there is no mention in the paper.

\section{Conclusion}

It is beyond the scope of this work to explain these. Any explaination would undoughtly need more experiments, and given it was straightforward to run these using OpenTTDLab, it should be straightfoward in future.
...
\includepdf[pages=-,scale=.8,pagecommand={}]{notebooks/03_scaling_01_run_experiment.pdf}
...
\includepdf[pages=-,scale=.8,pagecommand={}]{notebooks/03_scaling_02_analyse_results.pdf}

\chapter{trAIns versus Admiral - Python code to run experiments}
\label{chapter:trains-vs-admiral-run-experiments}

The Python notebook used to run the trAIns vs Admiral experiments of XX is below. This latest version is also available at \url{https://github.com/michalc/openttd-msc-dissertation/blob/main/notebooks/01_trains_ai_vs_admiral_ai_01_run_experiments.ipynb}

\includepdf[pages=-,scale=.8,pagecommand={}]{notebooks/01_trains_ai_vs_admiral_ai_01_run_experiments.pdf}

\chapter{trAIns versus Admiral - analyse results}
\label{chapter:trains-vs-admiral-analyse-results}

The Python notebook used to analyse results for the trAIns vs Admiral experiments of XX is below. This latest version is also available at \url{https://github.com/michalc/openttd-msc-dissertation/blob/main/notebooks/01_trains_ai_vs_admiral_ai_01_analyse_results.ipynb}

\includepdf[pages=-,scale=.8,pagecommand={}]{notebooks/01_trains_ai_vs_admiral_ai_02_analyse_results.pdf}
