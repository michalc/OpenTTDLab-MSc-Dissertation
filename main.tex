...
I conducted a brief investigation into how well OpenTTDLab scales with the number of worker processes by running the same set of 50 simulations, but distributing the work between 1 to 8 worker processes, and seeing how the runtime changes. I also did these tests when passing the full set of results back to client code, and for only passing only a small amount of data: the company value at each date. All the simulations were conducted on a 2020 Apple M1 with 8 CPU cores, 16GB of RAM, OpenTTDLab 0.0.72, OpenTTD 13.4, Python 3.11.0, running for seeds 0 to 49, and trAIns version 2.1 with MD5 hash of. The full code of the experiments can be seen Appendix X.

As can be seen from Figure X, runtime decreases as the number of processors increases, and so there is some useful parallelisation in terms of reducing the time for simulations. However, the amount that speedup increases as a processor is added decreases as the number of processors increases. And more specifically, up to 4 processors the scaling is just under linear, but after 4 processors the scaling is much worse. It can also be seen that the scaling is better when only a small amount of data is returned from the worker processes.
