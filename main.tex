...
\caption{A chart showing the balance over for 50 experiments (old version of OpenTTDLab - can't reproduce! Suspect because now AIs are started immediately, and so the random numbers are different? Or just realised, later experiments I ran using openTTD 13.1. Slightly different results for different versions of OpenTTD is interesting! I think justification for "exact" reproducibility?? Nice to have more comparison?)}
...
Output from an extremely basic AI
