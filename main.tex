...
These are not independent properties: reproducibility is unlikely to be achieved without repeatability \cite{hill2022reproducibility}, and replicability would be difficult to interpret in the absence of reproducibility \cite{nuijten2018verify}. While this highlights how the definitions are intertwined and therefore complex, it also means that a tool that eases repeatability indirectly eases reproducibility, and similarly a tool that eases reproducibility indirectly eases replication.
...
\section{Recommendations to achieve the 3Rs}

In spite of the historical confusion and vagueness of even the most recent definitions of the 3Rs, there are a number of relevant recommendations that in general can be followed to help authors achieve the 3Rs.
...
In spite of the aforementioned crisis, there are currently no set criteria to determine if a simulation study has been successfully replicated \cite{luijken2024replicability}. At the moment, it seems a qualitative comparison is the most reasonable.
...
For reproducibility the author-supplied artifacts must be clear and available. There were references to modified OpenTTD in \cite{wisniewski2011artificial}, but the modified versions does not seem to be available. Potentially the authors could be contacted to supply their own artifacts, but this is not ideal, nor offers a strong guarantee that the artifacts supplied were exactly as they were that generated the results.
