...
Of all these, using OpenTTD as a tool for researching algorithms is the primary focus of this project, and specifically the focus is the development of a tool, OpenTTDLab, that augments its existing features for repeatable, replicable, \& reproducible research. As a secondary and more speculative aim, it is hoped that the work here allows for OpenTTD to be investigated as a tool to simulate supply chain or transportation networks that could ultimately inform government policies, although a validation exercises would be required to do this, which is not part of the current work.

\section{Repeatability, Replicatability, \& Reproducibility}

The terms \emph{repeatability}, \emph{replicability}, and \emph{reproducibility} unfortunately do not historically have universally agreed meanings \cite{plesser_reproducibility_2018} and to add to the confusion, the Association for Computing Machinery (ACM) in 2020 swapped their definitions reproducibility and replicability \cite{association_for_computing_machiner_new_2020}. For the avoidance of doubt, I use the ACM's definitions that are current as of October 2023 \cite{association_for_computing_machiner_artifact_2020}. For some results of an experiment conducted by a team of authors, where some computer-based artefacts were created to conduct these, these are summarised as follows.
