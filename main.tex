...
It should be noted that these are not independent properties: reproducibility is cannot be achieved without repeatability \cite{hill2022reproducibility}, and replicability cannot be achieved without reproducibility. While this highlights how the definitions are intertwined and therefore complex, it also means that a tools that helps repeatability indirectly helps reproducibility, and similarly a tool that helps reproducibility indirectly helps replication.

It should also be noted that there is still some known vagueness to these definitions, especially in terms of what \emph{same result} means \cite{hill2022reproducibility}. I also note that the definition of artifact is limited to objects created by the authors; if research uses OpenTTD then it appears the research can be deemed replicated without OpenTTD itself having to be developed independently, simply because the it was already available, which seems an unexpected loophole in the definitions. It is beyond the scope of this project to deal with these issues in general; however I do clarify what I mean by artifacts, repeatability, reproducibility and replicability in the specific scope of research using OpenTTD later in the current chapter, informed by known barriers to achieving the 3Rs and an informal review of existing research using OpenTTD.

\section{Barriers to achieving the 3Rs}
