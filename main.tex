...
As can be seen from Figure X, runtime decreases as the number of processors increases, and so there is some useful parallelisation in terms of reducing the time for simulations. However, the speedup decreases as the number of processors increases. And more specifically, up to 4 processors the scaling is very almost linear, but after 4 processors the scaling is worse than linear. It can also be seen that the scaling is better when only a small amount of data is returned from the worker processes.

A reason for the noted difference when the number of processors is greater than 4 comes from noting that the cores are not identical. According to Apple [REF] 4 cores are high performance, but the other 4 are lower performance (but have higher efficiency in terms of power usage). The scaling profile found appears roughly consistent with the high performance cores being used in preference to the low performance cores.
...
\end{equation}
%
where $S_p$ is the speedup of the entire program, $p$ is the proportion of the program that has been parallelised, and $s$ is speedup of the parallelised part of the program. The most important consequence of this is that $\lim_{S\to\infty} S_p = \frac{1}{1-p}$. This means that example even if 95\% of a program is parallelised, then the maximum speedup is 20, even if thousands of processes are used.

This means that one way of increasing speedup is to reduce the amount that is not parallelised. One part that is effectively not parallelisable is the copy of data from the worker process back to the main process. As can be seen reducing this does increase the speedup.

To eliminsate this entire, the program avoiding using processes in the Python parts of OpenTTDLab, and instead use threads, which all an address space. However, Python suffers from the well-known Global Interpreter Lock (GIL) which means that even in multi-threaded code, only one thread can run at any one time, which could unfortunately increase the proportion of code that cannot be parallelised.
