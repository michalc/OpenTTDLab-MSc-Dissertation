...
\title{OpenTTDLab: A Python framework for repeatable, Replicable, \& Reproducible experiments using OpenTTD}
...
The aim of OpenTTDLab is to help studies that run OpenTTD to generate results without a human player; 7 existing studies have been found that do this: the work of Beuneker et al \cite{beuneker2019autonomous}, Bijlsma \cite{bijlsma2014evolving}, Konijnendijk \cite{konijnendijk2015mcts}, {Lakom{\`y} \cite{lakomy2020railroad}, Rios and Chaimowicz \cite{rios2009trains}, Wisniewski and Witt \cite{wisniewski2011artificial}, and Volna \cite{volna2017fuzzy}. Informally reviewing these in terms of how they have or have not seemingly achieved repeatability (the domain of the original researchers) or help achieve reproducibility and replicability (the domain of independent researchers), keeping in mind the barriers of the previous section and my own knowledge of the capabilities of OpenTTD, I found 8 problems. For brevity not all problems for all studies are listed.
...
\begin{descitem}{Insufficient detail on the version of configuration of OpenTTD - reproducibiliy}
\cite{bijlsma2014evolving} does not mention the version of OpenTTD (or the version of OpenGFX - as will be discussed a component that affects screenshots)
...
Possible a special case of configuration, but it merits special attention since to gain statistical results typically experiments would be run for a fixed configuration over a range of random seeds. However, only one study reported the random seeds used, \cite{beuneker2019autonomous}, including a discussion of them. This impacts repeatability, but specifically bitwise reproducibility: without the random seeds it is impossible to reproduce the results exactly.
\end{descitem}

\begin{descitem}{Insufficient detail of the algorithms to replicate results}
\end{descitem}

\end{enumerate}

Informally reviewing by reading the papers is certainly not as strong evidence as actually attempting to reproduce or replicate results, but an independent researcher reading through a draft manuscript has been given as a recommendation to find issues before publication with the strongest of the 3Rs, replicability \cite{luijken2024replicability}, so I argue it is reasonable to extend the process to all of the 3Rs and to base further actions off of such a review.

\section{Conclusion}

It can be costly to repeat experiments, or to make it easily reproducible and replicable; and there is unfortunately insufficient incentive to address these issues as a matter of course. Indeed, in all of the found research that uses OpenTTD to run experiments without human involvement, I identified 7 issues on this front, with no research completely issue-free. A framework to reduce the cost of these should help similar future research avoid adding to the well-known replication crisis, as will be discussed in the following chapters.
...
As discussed in Chapter \ref{chapter:introduction}, OpenTTD has been used to run experiments to help research a variety of algorithms. However, from my own reviews not a single in-scope study has been found that does not appear to have problems in at least one of the 3Rs of repeatability, reproducibility, or replicability. Given the wider replication crisis in many fields of science, this is a trend that should not continue, and a need to address this in future research using OpenTTD seems clear. The framework created as part of this work, OpenTTDLab, seeks to address this by reducing the cost of repeating experiments using OpenTTD, making it straightforward for others to then reproduce them, and also then aid replicating experiments through easing their own repetitions.

OpenTTD itself provides a rich and interesting setting for studies of algorithms. Its Squirrel-based AI framework, coupled with its tick-based and so deterministic simulator, as described in Chapter \ref{chapter:openttd-model-and-abilities}, provide an excellent foundation for bitwise repeatable studies and bitwise reproducible studies, and hopefully, if properly presented, replicable studies as well. While OpenTTD's limited supply chain is clearly a simplification of reality, and its economic model is unrealistic, it has been suggested there are realistic aspects to OpenTTD; while I did not attempt to validate OpenTTD as a simulator, I have found nothing to contradict that. If OpenTTDLab indeed provides a good framework for the 3Rs, it puts further research into these aspects in good stead.

From the first set of experiments in Chapter \ref{chapter:experiments-attempt-at-reproducing} I have further shown the difficulty in - attempting to reproduce existing results and while I did generate results, they did not match the ones originally reported. However, using OpenTTDLab and providing the artifacts for running an analysing results, I have shown that it is straightforward to repeat experiments, so hopefully for others to reproduce my work.

The second set of experiments in Chapter \ref{chapter:experiments-simple-parameterised-ai} have shown again that OpenTTDLab can be used to easily repeat experiments, and again. Going further in this case, the experiments hint that OpenTTD could be used to simulate more complex world properties relating to transportation, business, or logistics; obtaining results that appear to show risk-benefit trade offs.

The third set of experiments in Chapter \ref{chapter:experiments-scaling} show that OpenTTDLab can run experiments in parallel, leveraging multiple CPUs on a single commodity computer, albeit with scaling that should be able to be improved. While this might at first glance to not be important in terms of the 3Rs, it allows for more experiments to be repeated within the same time constraints, and so give more evidential weight to conclusions based on the results.

In all of these experiments the artifacts both in terms of running the experiments and analysing the results are included in appendices and are relatively short in terms of lines of code, showing that it should be feasible to include these in studies, or at least provide links to them as has been suggested.

I developed the framework in a highly agile mechanism as described in Chapter \ref{chapter:openttdlab-design-process-and-features}, iteratively writing and using the framework to create the results of Chapters \ref{chapter:experiments-attempt-at-reproducing}-\ref{chapter:experiments-scaling} attempting to avoid the issues described in Chapter \ref{chapter:introduction}, and so there are good reasons to beleive it is fit for purpose. However, according to ACM's definitions, reproducibility and replicability is the domain of multiple sets of authors: those originally conducting the research and those then attempting to reproduce or replicate it. To fully validate that OpenTTDLab can be a useful tool in this, studies that use OpenTTDLab will have to be attempted to be reproduced and replicated, much like the systematic attempts reported by Luijken et al. \cite{luijken2024replicability}. I have provided 3 sets of experiments in this work, and argued that they should be reproducible and replicable, but the true test of these properties are real attempts by other authors.

Finally there are numerous thing in OpenTTDLab itself that can be improved upon. Improving sclability has already been mentioned, but there are features. While screenshots can be generated of the map, this cannot currently be extracted in a way suitable for analysis. While in many ways the format is self-documenting, it is not fully documented in terms of what can be extracted and how - the examples just touch the service. Should the format be converted into a way suitable for more straightforward analysis, for example to convert experiments into a SQLite file? OpenTTDLab currently does not with with OpenTTD 14. Although OpenTTD 15 should be able to support the deterministic extract of results, the code to to this is not part of OpenTTDLab.

While there are many improvements that can be made in OpenTTDLab and further investigations made in terms of whether it does in fact achieve its aims, through this work I have provided evidence that OpenTTDLab at least provides the foundations of a good framework for conducting repeatable, reproducible and replicable research using OpenTTD. I sincerely hope it will be both used and improved upon.
...
\includepdf[pages=-,scale=.8,pagecommand={}]{notebooks/02_trains_ai_vs_admiral_ai_01_run_experiments.pdf}
...
\includepdf[pages=-,scale=.8,pagecommand={}]{notebooks/02_trains_ai_vs_admiral_ai_02_analyse_results.pdf}
